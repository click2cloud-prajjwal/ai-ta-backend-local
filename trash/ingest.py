...
class Ingest:
    ...
    def initialize_resources(self):
        ...
        if use_local_minio:
            ...
            bucket_name = os.getenv("MINIO_BUCKET", "uiuc-chatbot")
            verify_ssl = os.getenv("MINIO_SECURE", "False").lower() == "true"
        else:
            # Fallback to AWS S3
            endpoint_url = None
        ...
        try:
            ...
            logging.info(f"✅ Connected to S3/MinIO: {endpoint_url} (bucket={bucket_name})")
        except Exception as e:
            logging.error(f"❌ Failed to connect to MinIO/S3: {str(e)}")
        ...
        try:
            ...
            if uploaded_file_path and (not s3_paths or s3_paths[0] == ''):
                ...
                try:
                    ...
                    logging.info(f"📤 Uploading to MinIO: {s3_key}")
                    ...
                    logging.info(f"✅ Uploaded to MinIO: s3://{self.s3_bucket_name}/{s3_key}")
    ...
    def _ingest_single_pdf(self, s3_path: str, readable_filename: str, course_name: str):
        """
        ...
        try:
            ...
            tmp_pdf_path = os.path.join(tempfile.gettempdir(), os.path.basename(s3_path))
            with open(tmp_pdf_path, "wb") as f:
                self.s3_client.download_fileobj(
                    Bucket=self.s3_bucket_name, Key=s3_path, Fileobj=f
                )
            ...
            with fitz.open(tmp_pdf_path) as pdf_document:
                ...
                for i, page in enumerate(pdf_document):
                    ...
                    # Create Qdrant embedding payload
                    metadata = {
                        "course_name": course_name,
                    ...
                    if i == 0:
                        tmp_png_path = os.path.join(
                            tempfile.gettempdir(), f"{Path(s3_path).stem}_thumb.png"
                        )
                        ...
                        with open(tmp_png_path, "rb") as f_png:
                            ...
                            self.s3_client.put_object(
                                Bucket=self.s3_bucket_name, Key=s3_upload_path, Body=f_png
                            )
            ...
            print(f"❌❌ Error in PDF ingest (no OCR): `_ingest_single_pdf`: {e}")
            return {
                "status": "failure",
                "s3_path": s3_path,
                "error": str(e),
