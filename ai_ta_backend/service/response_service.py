...
# Import PosthogService safely (optional injection)
try:
...
class ResponseService:
    ...
    def __init__(self, posthog_service: Optional[PosthogService] = None):
        ...
        # Optional Posthog analytics service
        ...
        if self.posthog:
            logging.info("📊 PostHogService attached for analytics logging")
    ...
    def generate_response(
        ...
        try:
            ...
            # ✅ Automatically log analytics event to PostHog
            if self.posthog:
                try:
                    ...
                    logging.info("📊 Logged chat_response event to PostHog")
                except Exception as e:
                    logging.warning(f"⚠️ Failed to log PostHog event: {e}")
...
If the context doesn't contain relevant information, let the student know."""
    ...
    def generate_streaming_response(
        ...
        try:
            ...
            # ✅ Log streamed responses to PostHog after completion
            if self.posthog and full_response:
                try:
                    self.posthog.capture_event(
                        event_name="chat_response_streamed",
                        properties={
                            "project_name": course_name,
                            "model": self.chat_deployment,
                    ...
                    logging.info("📊 Logged streamed chat_response to PostHog")
                except Exception as e:
                    logging.warning(f"⚠️ Failed to log PostHog streamed event: {e}")
